...
from homeassistant.const import ATTR_MODE, ATTR_UNIT_OF_MEASUREMENT, CONF_ICON, CONF_MODE, CONF_NAME, SERVICE_RELOAD
...
async def async_setup(hass, config):
    ...
    homeassistant.helpers.service.async_register_admin_service(hass, DOMAIN, SERVICE_RELOAD, reload_service_handler, schema=RELOAD_SERVICE_SCHEMA)
...
async def _async_process_config(config):
    ...
    for (object_id, cfg) in config[DOMAIN].items():
        ...
        entities.append(InputText(object_id, name, initial, minimum, maximum, icon, unit, pattern, mode))
    return entities

class InputText(RestoreEntity):
    """Represent a text box."""

    def __init__(self, object_id, name, initial, minimum, maximum, icon, unit, pattern, mode):
    ...
    def state_attributes(self):
        ...
        return {ATTR_MIN: self._minimum, ATTR_MAX: self._maximum, ATTR_PATTERN: self._pattern, ATTR_MODE: self._mode}
    ...
    async def async_set_value(self, value):
        ...
        if len(value) < self._minimum or len(value) > self._maximum:
            _LOGGER.warning('Invalid value: %s (length range %s - %s)', value, self._minimum, self._maximum)
            return
        self._current_value = value
